...
public class Ciudad {
    ...
    public double promedioAgua() {
        double d = 0;
        d = estaciones.stream().map((estacion)
                -> estacion.promedioAgua()).reduce(d, (accumulator, _item)
                -> accumulator + _item);
    ...
    public double promedioTemp() {
        double d = 0;
        d = estaciones.stream().map((estacione)
                -> estacione.promedioTemp()).reduce(d, (accumulator, _item)
                -> accumulator + _item);
        d /= this.estaciones.size();
        return d;
    }
    ...
    public double promedioCarga() {
        double d = 0;
        d = estaciones.stream().map((estacione)
                -> estacione.promedioCarga()).reduce(d, (accumulator, _item)
                -> accumulator + _item);
        d /= this.estaciones.size();
        return d;
    }
    ...
    public String promedio() {
        return this.nombre + " " + this.promedioTemp() + " "
                + this.promedioAgua() + " " + this.promedioCarga();
    }
